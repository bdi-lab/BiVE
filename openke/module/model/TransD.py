	...
	def __init__(self, ent_tot, rel_tot, dim_e = 100, dim_r = 100, p_norm = 1, norm_flag = True, margin = None, epsilon = None):
		...
		if margin == None or epsilon == None:
			...
			if self.meta_rel_tot > 0:
				nn.init.xavier_uniform_(self.meta_rel_embeddings.weight.data)
				nn.init.xavier_uniform_(self.meta_rel_transfer.weight.data)
		else:
			...
			if self.meta_rel_tot > 0:
				self.meta_rel_embedding_range = nn.Parameter(
					torch.Tensor([(self.margin + self.epsilon) / self.dim_e]), requires_grad=False
				)
				nn.init.uniform_(
					tensor = self.meta_rel_embeddings.weight.data, 
					a = -self.meta_rel_embedding_range.item(), 
					b = self.meta_rel_embedding_range.item()
				)
				nn.init.uniform_(
					tensor = self.meta_rel_transfer.weight.data, 
					a= -self.meta_rel_embedding_range.item(), 
					b= self.meta_rel_embedding_range.item()
				)	
		...
		if self.meta_rel_tot > 0:
			...
			nn.init.xavier_uniform_(self.W_transfer.weight.data)
