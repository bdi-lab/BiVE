...
from openke.module.model import BiVE_BiQUE_add
...
try:
	...
	BiVE_BiQUE_add.load_checkpoint('./checkpoint/{}/BiVE_BiQUE_add/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))
except:
	print("No checkpoint available!")
		
	# define the loss function
	model = NegativeSampling(
		model=BiVE_BiQUE_add, 
		loss=SoftplusLoss(),
	...
	trainer.run()
		os.makedirs("./checkpoint/{}/BiVE_BiQUE_add".format(args.data), exist_ok=True)
		BiVE_BiQUE_add.save_checkpoint('./checkpoint/{}/BiVE_BiQUE_add/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))

if args.lp:
	test_dataloader = TestDataLoader("./benchmarks/{}/base/".format(args.data), "link")
	tester = Tester(model=BiVE_BiQUE_add, data_loader=test_dataloader, use_gpu=True)
...
if args.lp:
	os.makedirs("./result_test/{}/BiVE_BiQUE_add/LP".format(args.data), exist_ok=True)
	with open("./result_test/{}/BiVE_BiQUE_add/LP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("LP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.tp:
	os.makedirs("./result_test/{}/BiVE_BiQUE_add/TP".format(args.data), exist_ok=True)
	with open("./result_test/{}/BiVE_BiQUE_add/TP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("TP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.clp:
	os.makedirs("./result_test/{}/BiVE_BiQUE_add/CLP".format(args.data), exist_ok=True)
	with open("./result_test/{}/BiVE_BiQUE_add/CLP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("CLP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))